...
def train(device, epochs, batch_size, lr):
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=lr, weight_decay=1e-3, betas=(0.9, 0.99)
    )
    sigma_data = 0.5
    ...
    for epoch in range(epochs):
        ...
        for batch, _ in progress_bar:
            ...
            sigma = torch.randn(images.shape[0], device=images.device).reshape(
                -1, 1, 1, 1
            )
            ...
            loss = (1 / torch.exp(logvar)) * torch.square(
                (pred_v_t - v_t) / sigma_data
            ) + logvar
            loss = loss.mean()
        ...
        with torch.no_grad():
            for curr_t in test_t:
                ...
                logvar = logvar.view(-1, 1, 1, 1)
                pred_x0 = torch.clamp(
                    torch.cos(t) * x_t - torch.sin(t) * sigma_data * pred,
                    min=-0.5,
                    max=0.5,
                )
