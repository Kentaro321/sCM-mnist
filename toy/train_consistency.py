...
@click.option(
    "--swap-cos", default=True, help="Swap cos for sin in the gradient calculation"
)
def train(device, epochs, batch_size, lr, swap_cos):
    ...
    sampling_timesteps = torch.arctan(
        calc_karras_sigmas(sigma_min=0.002, sigma_max=80, steps=1000, rho=7)
        / sigma_data
    ).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch in progress_bar:
            ...
            if not swap_cos:
                ...
                # Note that F_theta_grad is already multiplied by sin(t) cos(t) from the tangents. Doing it early helps with stability.
                second_term = -r * (
                    torch.sin(t) * torch.sin(t) * x_t + sigma_data * F_theta_grad
                )
            g = g + second_term
        ...
        with torch.no_grad():
            trajectory_guess = (
                -sigma_data
                * model_wrapper(
                    x_t / sigma_data, torch.full_like(x_t, sampling_timesteps[0])
                )[0]
            )
        ...
        for t in sampling_timesteps:
            ...
            if not use_exact_trajectory:
                x_t = (
                    torch.cos(t - next_t) * x_t
                    - torch.sin(t - next_t) * sigma_data * F_theta
                )
            else:
        ...
        trajectory_x0.append(
            x_t.cpu().numpy()
        )  # At t=0, x_t is the final x0 prediction
        trajectory_grad.append(np.zeros_like(x_t.cpu().numpy()))  # At t=0, grad is 0

        consistency_error = np.abs(
            trajectory_guess.cpu().numpy() - trajectory_x0[-1]
        ).mean()
        ...
        discrete_grad[:-1] = (
            -np.cos(trajectory_t[:-1, None])
            * (trajectory_x0[1:] - trajectory_x0[:-1])
            / (trajectory_t[1:, None] - trajectory_t[:-1, None])
        )
