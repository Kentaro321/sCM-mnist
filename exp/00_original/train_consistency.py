...
@click.option(
    "--extra-plots",
    is_flag=True,
    help="Generate additional training plots",
    default=True,
)
def train(device, epochs, batch_size, lr, extra_plots):
    ...
    if os.path.exists("model.pt"):
        ...
        model.logvar_linear = nn.Linear(
            model.logvar_linear.in_features, model.logvar_linear.out_features
        ).to(device)
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=lr, weight_decay=1e-2, betas=(0.9, 0.99)
    )
    sigma_data = 0.5
    ...
    for epoch in range(epochs):
        ...
        for batch, class_label in progress_bar:
            ...
            weight = 1
            loss = (weight / torch.exp(logvar)) * torch.square(
                F_theta - F_theta_minus - g
            ) + logvar
            loss = loss.mean()
        ...
        z = torch.randn(16, 1, 28, 28, generator=torch.Generator().manual_seed(42)).to(
            device
        )
        ...
        with torch.no_grad():
            ...
            z = torch.randn(
                16, 1, 28, 28, generator=torch.Generator().manual_seed(43)
            ).to(device)
            t1_exp = t1.view(-1, 1, 1, 1)
            x_t = torch.sin(t1_exp) * z * sigma_data + torch.cos(t1_exp) * pred_x0
            pred_x0 = torch.clamp(
                torch.cos(t1_exp) * x_t
                - torch.sin(t1_exp) * sigma_data * model(x_t / sigma_data, t1),
                min=-0.5,
                max=0.5,
            )
        ...
        z = (
            torch.randn(1, 1, 28, 28, generator=torch.Generator().manual_seed(42)).to(
                device
            )
            * sigma_data
        )
        z = z.repeat(16, 1, 1, 1)  # Repeat the same noise 16 times
        t = torch.linspace(0, 1.56454, 16, device=device).view(
            -1, 1, 1, 1
        ...
        with torch.no_grad():
            pred_x0 = torch.clamp(
                torch.cos(t) * x_t
                - torch.sin(t) * sigma_data * model(x_t / 0.5, t.flatten()),
                min=-0.5,
                max=0.5,
            )
            plt.figure(figsize=(12, 12))
        ...
        if model_pretrained is not None and epoch % 5 == 0:
            sampling_timesteps = torch.arctan(
                calc_karras_sigmas(sigma_min=0.002, sigma_max=80, steps=64, rho=7)
                / sigma_data
            ).to(device)
            trajectory_x0 = []
            trajectory_xt = []
            ...
            x_t = (endpoints[0] * alphas + endpoints[1] * (1 - alphas)) / torch.sqrt(
                alphas**2 + (1 - alphas) ** 2
            )
            with torch.no_grad():
                trajectory_guess = (
                    -sigma_data
                    * model_wrapper(
                        x_t / sigma_data, sampling_timesteps[0].repeat(x_t.shape[0])
                    )[0]
                )
            ...
            for t in tqdm(sampling_timesteps):
                ...
                with torch.no_grad():
                    dxt_dt = sigma_data * model_pretrained(
                        x_t / sigma_data, t_repeated.flatten()
                    )

                v_x = (
                    torch.cos(t_repeated) * torch.sin(t_repeated) * dxt_dt / sigma_data
                )
                ...
                g = g + second_term
                trajectory_grad.append(
                    torch.sqrt(torch.square(g).sum(dim=(1, 2, 3))).cpu().numpy()
                )
            ...
            discrete_grad[:-1] = (
                -np.cos(trajectory_t[:-1])[:, None, None, None, None]
                * (trajectory_x0[1:] - trajectory_x0[:-1])
                / (trajectory_t[1:] - trajectory_t[:-1])[:, None, None, None, None]
            )
